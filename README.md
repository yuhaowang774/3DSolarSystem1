# 3D Solar System · 星穹漫游

基于 **Vue 3 + Vite + Three.js** 构建的 3D 交互式太阳系模拟与天文科普平台，采用 **SpaceX 任务控制台**视觉风格。

> 精确天文计算 · 实时时间同步 · 时间穿越 · 天体科普 · 日下点校准法 · 一镜到底运镜

---

## ✨ 功能特性

- **精确天文计算**：基于 J2000.0 历元轨道六要素与开普勒方程求解，实时计算八大行星与月球的真实轨道位置。
- **日下点差量校准法**（核心创新）：独创地球日照同步算法，使模型地球的昼夜分界线与现实 UTC 时间完全对应。
- **时间控制系统**：支持实时同步、时间倒流/静止/前进（57 档位，秒级 → 年级）、播放/暂停。
- **一镜到底运镜**：一键播放从太阳出发、依次飞掠八大行星与月球、拉出太阳系总揽、直至银河系全貌的连续旅程；全程一条样条路径 + 连续速度剖面 + 惯性跟随，无剪切拼接，播放中可随时跳过。
- **天体信息面板**：点击任意天体查看物理参数、轨道数据、神话背景、探测历史等科普信息。
- **搜索导航**：中英文检索天体，平滑飞行聚焦。
- **相机锁定跟随**：聚焦后相机持续跟随天体公转/绕行轨迹，`Esc` 或徽标 `RELEASE` 解除。
- **标签遮挡检测**：射线检测实现标签被天体遮挡时自动淡出。
- **视距剔除**：按相机距离自动隐藏远处天体，保证流畅渲染。
- **宇宙呈现（NASA Eyes 风格）**：真实星表三维星野（HYG 亮星按真实距离分布、按色指数着色）叠加银河系照片面片；持续缩小可见恒星散开、银河盘面渐显，最终太阳系落在 40 kpc 银河图幅上的一个亮点。
- **任务读数**：右下角实时显示 FPS 与镜头速度（万 km/s），便于定位卡顿。
- **响应式交互**：鼠标 + 触屏双模式，适配移动端。

## 🛰️ 技术栈

| 技术 | 版本 | 用途 |
| --- | --- | --- |
| Vue | ^3.5 | 组件化 UI 层 |
| Vite | ^6.0 | 构建工具 / 开发服务器 |
| Three.js | ^0.170 | 3D 渲染引擎 |
| GSAP | ^3.15 | 运镜时间轴驱动 |
| CSS2DRenderer | 原生 | 天体标签（HTML 叠加层） |

## 📁 目录结构

```
3DSolarSystem1/
├── index.html                  # Vite 入口
├── vite.config.js              # Vite 配置
├── wrangler.toml               # Cloudflare Pages 配置
├── assets/                     # 原始纹理素材（jpg/png，不参与构建）
├── scripts/
│   └── optimize-textures.mjs   # 纹理优化：assets/ → public/assets/ 转 webp 压缩
├── public/
│   └── assets/                 # 优化后的 webp 纹理，构建时原样拷贝
└── src/
    ├── main.js                 # Vue 应用入口
    ├── App.vue                 # 根组件（组合各 UI 层 + 3D 场景）
    ├── core/
    │   ├── SolarSystem.js      # Three.js 场景核心（天体/轨道/标签/相机/拾取/动画循环）
    │   ├── CameraDirector.js   # 一镜到底运镜播放器（样条路径 + 速度剖面 + 惯性弹簧）
    │   ├── shots.js            # 运镜分镜数据（TOUR：各站环绕弧与转移参数）
    │   ├── starfield.js        # 真实星表三维星野（按真实距离放置与着色）
    │   ├── galaxy.js           # 银河系照片面片（按视距淡入，UV 锚定太阳位置）
    │   └── bright_stars.json   # 亮星数据（HYG v4.2 子集，517 颗，mag ≤ 4.0）
    ├── components/
    │   ├── SolarScene.vue       # 挂载 SolarSystem 类的画布容器
    │   ├── TopBar.vue           # 顶部品牌栏 + 运镜按钮 + 信息面板开关
    │   ├── SearchBar.vue        # 天体搜索导航
    │   ├── TimeController.vue   # 时间控制滑块
    │   ├── InfoPanel.vue        # 天体科普信息面板
    │   └── LoadingOverlay.vue   # SpaceX 风格加载遮罩
    ├── composables/
    │   └── useTimeController.js # 时间控制逻辑（状态 + 时间步长计算）
    ├── store/
    │   └── useStore.js          # 响应式共享状态（Vue ↔ Three.js 通信桥梁）
    ├── styles/
    │   └── global.css           # SpaceX 风格设计系统（CSS 变量 / 字体 / 通用排版）
    └── js/
        ├── utils.js             # 天文计算、天体创建、地球昼夜 Shader、日下点校准法
        └── dats.js              # 天体数据配置（轨道要素 + 科普文案 + 中文名）
```

## 🚀 本地开发

```bash
# 安装依赖
npm install

# 启动开发服务器（热更新，默认 http://localhost:5173）
npm run dev

# 生产构建（输出到 dist/）
npm run build

# 本地预览构建产物
npm run preview

# 部署到 Cloudflare Pages（build + wrangler pages deploy dist）
npm run deploy

# （可选）重新生成 webp 纹理：sharp 为按需安装的开发工具，未列入常规依赖
npm i -D sharp && npm run optimize:textures
```

## 🎮 操作指南

| 操作 | 鼠标 | 触屏 |
| --- | --- | --- |
| 旋转视角 | 左键拖动 | 单指拖动 |
| 缩放 | 滚轮 | 双指捏合 |
| 平移 | 中键拖动 | 双指拖动 |
| 聚焦天体 | 点击天体本体 / 标签 | 点击天体本体 / 标签 |
| 查看信息 | 点击后自动展开信息面板 | 同左 |
| 打开信息面板 | 点击右上角 ⓘ 按钮 | 同左 |
| 一镜到底运镜 | 点击右上角 `CINEMATIC ▶`（播放中为 `SKIP ▶▶`） | 同左 |
| 解除跟随 | `Esc` 键 / 点徽标 RELEASE | 点徽标 RELEASE |

点击天体后，相机会以 easeInOutCubic 缓动平滑飞行至该天体的安全视距；时长随飞行距离动态伸缩（约 0.7s–3.2s，对数增长），近距离轻快、跨行星从容。悬停在天体上时光标变为手型。

聚焦后相机进入**锁定跟随**状态：此后无论实时同步还是任意倍速时间流动，相机都会持续跟随该天体的公转/绕行轨迹，天体始终保持在画面中心。屏幕顶部会出现 `LOCKED · 天体名` 徽标提示当前锁定状态，按 `Esc` 或点击徽标上的 `RELEASE` 可解除锁定、恢复自由漫游。关闭信息面板**不会**解除跟随。

底部中央为时间控制器，可展开完整面板，通过滑块或按钮调节时间流速与方向。

## 🔭 核心算法

### 日下点差量校准法

位于 `src/js/utils.js`，核心思路是「先测现状 → 再算目标 → 最后求差旋转」：

1. **测现状**：从地球模型的世界矩阵反算太阳方向在本体坐标系中的指向，得到模型当前日下点经度；
2. **算目标**：由 UTC 时间求平太阳直射经度，再叠加高精度均时差修正（由太阳几何平黄经与视赤经之差求得，误差约 ±5 秒），得到真实日下点经度；
3. **求差旋转**：两者求差得到校准角，初始化时应用一次；此后每帧以增量方式更新地球自转，避免累积误差。

最终使 3D 模型的昼夜分界线与现实地球的 UTC 时间完全同步。

### 轨道位置计算

基于 J2000.0 历元的轨道六要素（半长轴、偏心率、倾角、平黄经、近日点黄经、升交点黄经），解开普勒方程（牛顿迭代至 1e-12）求偏近点角，再转换到天球赤道坐标系，并支持每世纪长期变化率。精度增强包括：

- **ΔT 修正**：轨道要素以 TT 为时间引数，UTC 经 ΔT（Espenak–Meeus 多项式，约 69 s）转换，消除月球约 38″、行星约 3″ 的引数误差；
- **月球周期摄动**：叠加出差、二均差、年差等主要项（Meeus 第 47 章截断），月球经度误差从约 ±1.5° 降至约 ±0.2°；
- **高精度均时差**：由太阳几何平黄经与视赤经之差直接求得（Meeus 第 25/28 章，含中心差、光行差与章动主要项），误差约 ±5 秒（旧经验简式约 ±1 分钟），日下点经度即昼夜分界线的准确性由此决定。

### 地球昼夜与大气

地球使用自定义 Shader 材质：昼半球卫星贴图 + 夜半球城市灯光 + 晨昏线暖色散射 + 海面镜面耀斑，外层叠加菲涅尔大气辉光。太阳方向在**视空间**逐帧计算，规避大世界坐标的 float32 量化误差在近景造成的照明抖动。

### 一镜到底运镜

`src/core/CameraDirector.js` 将 `shots.js` 的分镜数据构建为一条 CatmullRom 样条：环绕段按轨道速度慢速绕行，转移段采用钟形速度剖面（时间均匀采样，出入速度与圈端部速度衔接），全程速度连续、无停顿拼接；位置与视线再经指数趋近的惯性层跟随，转弯时按角速度自适应刚度并叠加压坡度滚转。

### 轨道线的「偏心」现象

轨道线是按真实轨道要素绘制的**开普勒椭圆**：太阳位于**焦点**而非几何中心。椭圆中心相对太阳的偏移量 `c = a × e`，随轨道半长轴增大（天王星最明显，约 0.9 AU）。这与 NASA JPL 的轨道数据一致，是真实的天文几何而非渲染偏差——行星始终精确运行在轨道线上。

## 📄 许可证

本项目用于教育与科普目的。

## 🙏 致谢

参考 [3D-Solar-System-Model](https://github.com/LinkZY3471/3D-Solar-System-Model) 的部分实现进行改写。
