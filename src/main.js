...
// 扩展模块：使用 "three/addons/" 前缀，对应映射路径 "/node_modules/three/examples/jsm/"
...
// dat.gui 若通过 npm 安装，无需修改（若通过 CDN 需补充 importmap 配置）
...
const saturnRing = createRing(
  planetData.saturn.ringName,
  planetData.saturn.innerRing,
  planetData.saturn.outerRing
);
const uranusRing = createRing(
  planetData.uranus.ringName,
  planetData.uranus.innerRing,
  planetData.uranus.outerRing
);
const neptuneRing = createRing(
  planetData.neptune.ringName,
...
const mercury = createPlanet(
  planetData.mercury.name,
  planetData.mercury.radius
);
...
const jupiter = createPlanet(
  planetData.jupiter.name,
  planetData.jupiter.radius
);
...
const neptune = createPlanet(
  planetData.neptune.name,
  planetData.neptune.radius
);
...
const updatePlanets = (delta, speed) => {
  for (const name in planets) {
    ...
    // 自转速度
    rotationSpeeds[name] =
      ((2 * Math.PI) / (planetData[name].day * 3600)) * speed;
