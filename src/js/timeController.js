...
let isCollapsed = true; // 时间控制器是否处于折叠状态（默认折叠）
 ...
 * @param {string} size - 控制柄尺寸
 * @param {string} color - 控制柄颜色
 * @param {string} boxShadow - 控制柄阴影样式
 */
function setSliderThumbStyle(slider, size, color, boxShadow = "none") {
  // 清除所有之前的样式，避免残留
...
export function initTimeController(options = {}) {
  ...
  container.appendChild(timeDisplay); // 时间显示直接放在容器中，不在左右容器内
  ...
  // 添加时间显示点击事件
  timeDisplay.addEventListener("click", toggleCollapse);
...
function createContainer() {
  ...
  div.className = "time-controller-container collapsed";
  return div;
}
...
function createSpeedSlider() {
  ...
  function updateSliderThumbColor() {
    ...
    if (currentValue < 0) {
      // 倒退状态 - 红色
      setSliderThumbStyle(slider, "20px", "#ff4444");
    } else if (currentValue > 0) {
      // 前进状态 - 蓝色
      setSliderThumbStyle(slider, "20px", "#2196F3");
    } else {
      // 静止状态 - 灰色
      setSliderThumbStyle(slider, "20px", "#9E9E9E");
    }
  }
  ...
  slider.addEventListener("mouseover", () => {
    const currentValue = getTimeScale();
    if (currentValue < 0) {
      setSliderThumbStyle(slider, "24px", "#ff6666", "0 0 12px #ff6666");
    } else if (currentValue > 0) {
      setSliderThumbStyle(slider, "24px", "#4da6ff", "0 0 12px #4da6ff");
    } else {
      setSliderThumbStyle(slider, "24px", "#bdbdbd", "0 0 12px #bdbdbd");
    }
  });
...
function handlePresetChange(delta, btn) {
  ...
  if (newIndex >= 0 && newIndex < timePresets.length) {
    ...
    const currentValue = getTimeScale();
    if (currentValue < 0) {
      setSliderThumbStyle(speedSlider, "20px", "#ff4444");
    } else if (currentValue > 0) {
      setSliderThumbStyle(speedSlider, "20px", "#2196F3");
    } else {
      setSliderThumbStyle(speedSlider, "20px", "#9E9E9E");
    }
...
// 日期时间显示功能通过createTimeDisplay和updateTimeDisplay函数实现
...
function setCollapsed(collapsed) {
  ...
  if (container) {
    if (isCollapsed) {
      ...
      if (timeDisplay) {
        timeDisplay.title = "点击查看完整时间控制面板";
      }
    } else {
      container.classList.remove("collapsed");
      container.classList.add("expanded");
      if (timeDisplay) {
        timeDisplay.title = "点击收起时间控制面板";
      }
    }
  }
}
...
/**
 * 创建LIVE按钮
 * @returns {HTMLButtonElement} 创建的LIVE按钮元素
 */
...
function createTimeDisplay() {
  ...
  div.title = "点击查看完整时间控制面板";
  // 初始显示当前时间
  updateTimeDisplay(new Date());
  return div;
}

// updateTimeDisplay函数已作为导出函数定义在上面
