...
`;
loaderText.textContent = "加载纹理资源中...";
loaderContainer.appendChild(loaderText);

// 定义加载状态变量
let earthTextureLoaded = false;
let otherTexturesLoaded = false;

// 加载完成检测函数
function checkAllLoaded() {
  if (earthTextureLoaded && otherTexturesLoaded) {
    loaderText.textContent = "加载完成！";
    setTimeout(() => {
      loaderContainer.style.opacity = 0;
      setTimeout(() => {
        document.body.removeChild(loaderContainer); // 移除加载页面
        // 继续执行原来的动画循环
        animate(); // 加载完成后启动动画循环
      }, 500);
    }, 800);
...
setTimeout(() => {
  ...
  loaderText.textContent = "地球纹理加载完成，正在加载其他天体资源...";
  checkAllLoaded();

  // 模拟其他纹理加载
  setTimeout(() => {
    otherTexturesLoaded = true;
    checkAllLoaded();
  }, 800);
}, 1500);
...
orbitGroup.visible = true; // 轨道默认始终显示
