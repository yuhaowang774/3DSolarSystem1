...
点击天体后，相机会以 easeInOutCubic 缓动（1.2s）平滑飞行至该天体的安全视距；悬停在天体上时光标变为手型。

聚焦后相机进入**锁定跟随**状态：此后无论实时同步还是任意倍速时间流动，相机都会持续跟随该天体的公转/绕行轨迹，天体始终保持在画面中心。屏幕顶部会出现 `LOCKED · 天体名` 徽标提示当前锁定状态，按 `Esc` 或点击徽标上的 `RELEASE` 可解除锁定、恢复自由漫游。关闭信息面板**不会**解除跟随。
